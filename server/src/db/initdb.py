"""Initialize the users database with random student data."""
import sqlite3
...
def get_cursor(
    *,  # forces keyword-only arguments, makes calls like get_cursor(as_dict=True)
    ...
    """
    Return a new cursor and connection for the users database.
    ...
    """
    db_: Connection = sqlite3.connect("src/db/users.db")
    if as_dict:
